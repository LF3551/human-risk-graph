...
class HumanRiskGraph:
    ...
    def __init__(self, people: List[Dict], dependencies: List[Dict]):
        ...
        self.edge_types = {
            (dep["from"], dep["to"]): dep.get("type", "unknown")
            for dep in dependencies
        }
        self.weights = {
            (dep["from"], dep["to"]): dep.get("weight", 0.5)
            for dep in dependencies
        }
    ...
    def calculate(self, 
                  ...
                  beta: float = 0.3,
                  gamma: float = 0.3,
                  critical_threshold: float = 0.7) -> Dict:
        """
        ...
        critical_nodes = [
            node for node, crit in self.criticality.items()
    ...
    def simulate_node_removal(self, node_id: str) -> Dict:
        ...
        temp_edge_types = {
            k: v for k, v in self.edge_types.items()
